...
import { PriceList, PriceModel } from "./stores/interface";
import {
  FilterDefaultModel,
  ModalAction,
...
import { columnsPrice } from "./stores/columns";
...
const Category = () => {
  ...
  const [resData, setResData] = useState<PriceModel | undefined>(undefined);
  ...
  const fetchDataList = async () => {
    setIsLoading(true);
    const res = await priceApi.list(paramFilter, token);
    setIsLoading(false);
    if (res?.statusCode == 200) setResData(res as PriceModel);
  ...
  const onDelete = async (id: number) => {
    ...
    setIsLoading(true);
    try {
      const res = await priceApi.delete(id);
      if (res?.success) {
